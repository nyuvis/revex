...
ES.factory('es', function (esFactory) {
    ...
    self.query = function (filters) {
        ...
        if (self.hasSearch(filters)) {
            ...
            q.push("document.text:(" + filters.search + ")");
        }
        ...
        if (self.filterDate(filters)) {
            q.push("document.created:[" + filters.date.from.stamp + " TO " + filters.date.to.stamp + "]");
        }
    ...
    self.getDocuments = function (filters, from) {
        ...
        return self.client().search({
            ...
            docs = docs.map(function (d) {
                var newD = d._source;
                if (!d.highlight) {
                    newD.text = newD.document.text;
                } else {
                    newD.text = d.highlight["document.text"][0];
                }
                newD.t1 = newD.text[newD.text.length - 1];
                newD.t2 = newD.document.text[newD.document.text.length - 1];
                newD.id = d._id;
                if (newD.text[newD.text.length - 1] !== newD.document.text[newD.document.text.length - 1]) {
                    newD.text = newD.text + "...";
                }
    ...
    self.getDocument = function (filters, fragSize) {
        ...
        return self.client().search({
            ...
            docs = docs.map(function (d) {
                var newD = d._source;
                newD.text = d.highlight["document.text"][0];
                newD.high = d.highlight["document.text"];
                newD.t1 = newD.text[newD.text.length - 1];
                newD.t2 = newD.document.text[newD.document.text.length - 1];
                newD.id = d._id;
                if (newD.text[newD.text.length - 1] !== newD.document.text[newD.document.text.length - 1]) {
    ...
    self.getTerms = function (filters, type) {
        var size = 50, field = "document.text",
            re = /(\w+)/g,
